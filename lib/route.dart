...
Future<void> fetchData() async {
  try {
    final response = await http.get(Uri.parse('http://10.0.0.155:5001/test/'));
...
postData(String firstName, String lastName, String email,
  ...
  try {
    print('In PostData');
    var url = Uri.http('10.0.0.155:5001', '/test/add');
...
Future<void> updateTwoFAType(String email, String twoFAType) async {
  try {
    print('In UpdateData');
    var url = Uri.http('10.0.0.155:5001', '/test/update/$email');
